...
<br>
<br>
1. Judging from the cumulative pct of explained variance in the first Eigenvalue table, we will need to explain at least the top 6 components of variance, as they in total account for 52% of the variance in the data.

<br>
<br>
2. The highest weights of the first principal component portfolio weighs the most on:
<br> 5 yr T-Notes US (.93)
<br> 10 yr T-Notes (.93)
<br> Treasury Bonds (.86)
<br> 2 yr T-Notes US (.85)
<br> Canadian Bond (.79)
<br> Eurodollar (.74)

<br>
<br>
3. Based on the weights that each component has on the futures, I would explain the top 10 components as the following:
<br> 1. US Treasury Market
<br> 2. Foreign Currency
<br> 3. European Indices
<br> 4. US Indices
<br> 5. Refined Oil
<br> 6. Industrial Metals
<br> 7. US Treasury Market
<br> 8. Grains
...
<br>
<br>
4. First create the variables for bull and crisis data.
<br>
```{r}
...
<br>Bull Data<br>
```{r}
show_data = round(cor(bull_data),2)
iprint.df(show_data, scale=TRUE)
```
<br>
```{r echo=TRUE}
Variance_Explained_Table_results_bull<-PCA(bull_data, graph=FALSE)
Variance_Explained_Table_bull<-Variance_Explained_Table_results_bull$eig
...
<br>
```{r echo=TRUE}
eigenvaluesbull  <- Variance_Explained_Table_bull[, "Eigenvalue"]
```
<br>Scree plot:<br>
```{r}
df           <- cbind(as.data.frame(eigenvaluesbull), c(1:length(eigenvaluesbull)), rep(1, length(eigenvaluesbull)))
colnames(df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(df, id="components"))
```
<br>Crisis Data<br>
```{r}
show_data = round(cor(crisis_data),2)
iprint.df(show_data, scale=TRUE)
```
<br>
```{r echo=TRUE}
...
<br>
```{r echo=TRUE}
eigenvaluescrisis  <- Variance_Explained_Table_crisis[, "Eigenvalue"]
```
<br>Scree plot:<br>
```{r}
df           <- cbind(as.data.frame(eigenvaluescrisis), c(1:length(eigenvaluescrisis)), rep(1, length(eigenvaluescrisis)))
colnames(df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(df, id="components"))
```
<br><br>
5. From the data, I noticed that the percentage of the bear data is much more concentrated in component 1. This means that the value of the futures are more heavily correlated on the US Treasury market. This could be perhaps because investors are more invested more on US Treasury during recessions, because the returns are more certain. On the other hand, during the bull market, the concentration is much less at the top. US Treasury components do not take as much of a majority percentage of correlation than in the bear market, perhaps because the distribution of wealth is more dispersed because investors have more confidence in the other components.
...
<br><br>
1. The first step of the code was to normalize the futures table, showing 0 values for all cells. Next, the MAfreq variable creates a range of numbers at starting with 10 at increments of 20 until 250. Next, an iterator is created that goes through each column and adds a positive or negative sign, depending on the future data for each column. This acts as the "vote" sign and their predicted trajectory for that day. Lines 193-198 add $1 to each entry, which is then multiplied to the vote value. Finally, the returns are calculated and shown.
<br><br>
2. In order to manipulate the strategy, I created new variables with the same code structure, but modified the levers that are shaping the strategy. One example is the MAfreq range. Here, I extended the moving average, and extended the increment. I also changed the scrub time period, moving back to 5 days instead of 2. As you can see in the plots below, it modifies the strategy slightly.
```{r echo=TRUE, tidy=TRUE}
signal_used_test = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,300,by=50)
for (iter in 1:length(MAfreq))
...
# Now create the returns of the strategy for each futures time series
strategy_by_future_test <- scrub(shift(signal_used_test,5)*futures_data) # signal 2 days ago
# finally, this is our futures trend following strategy
...
iprint.df(show_data)
```
Thus, it is very important to understand the parameters and assumptions of your strategies.
*
*
*
*
