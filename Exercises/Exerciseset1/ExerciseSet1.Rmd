...
<br>
<br>
1. mytickers = c("SPY", "AAPL") from dataSet1.R
<br>
<br>
2. Cumulative return: round(100*sum(StockReturns[,"AAPL"]),1) 
<br>[1] 597.3 
<br>Average daily return: round(100*mean(StockReturns[,"AAPL"]),3) 
<br>[1] 0.148 
<br>Standard deviation: round(100*sd(StockReturns[,"AAPL"]),3) 
<br>[1] 2.389 
<br>
<br>
...
ncol(transposedData) 
[1] 4034 
It is similar to the above command that we executed for StockReturns. However, you will notice that the number of rows and columns are switched between the two matrices, due to the t() command. 
<br>
<br>
2. 
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
...
names(newport) <- colnames(transposedData) 
pnl_plot(newport)
```
<hr>
<hr>
...
<br>
<br>
1. We change the start date. 
<br> startDate = "2001-01-01"
<br>
2. Change the mytickers part of the code, adding more symboles to mytickers = c("SPY", "AAPL","YHOO","GOOG","GS","TSLA","FB","MSFT") 
<br>
