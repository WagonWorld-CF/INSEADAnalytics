...
<br>
<br>
_1. After entering source("dataSet1.R"), the code returns a line: [1] "\nDownloading ticker  AAPL  ..." This is where Apple returns are being downloaded from the dataSet1.R file._ 
<br>
<br>
_2. round(100*sum(StockReturns[,"AAPL"]),1)_
<br>
_[1] 597.3_
<br>
_round(100*mean(StockReturns[,"AAPL"]),3)_
<br>
_[1] 0.148_
<br>
_round(100*sd(StockReturns[,"AAPL"]),3)_
<br>
_[1] 2.389_
<br>
<hr>
...
[1] 4034
<br>
It is similar to the above command that we executed for StockReturns. However, you will notice that the number of rows and columns are switched between the two matrices, due to the t() command._
<br>
<br>
_2. transposedData = t(StockReturns)
<br>
newport = apply (transposedData,2,mean)
<br>
names(newport) <- rownames(StockReturns[,c("SPY","AAPL")])
<br>
pnl_plot(newport)
<hr>
<hr>
...
<br>
<br>
_1. In order to re-do the analysis of the data, we must change the start date. From the head(StockReturns,1) command, the earliest date listed is 2001-01-03. Simply change this number to 2001-01-01._
<br>
<br>
...
<br>
<br>
_2. It provides an error because these two datasets are not in the same format. myData is one column with 20 rows that are comma separated in a string format, where as StockReturns is a 3 column by 20 row matrix. Thus, when attempting to do the add function, it is trying to add a string of words to a date number, which is impossible. In order for this to make sense, myData needs to be separated into different columns, based on the comma separation._
<hr>
<hr>
